...
/// single/double quoting and backslash escapes, plus two line-continuation
/// styles people paste from real terminals: a trailing backslash-newline
/// (bash/zsh/sh) and a trailing caret-newline (Windows <c>cmd.exe</c>, the
/// form "Copy as cURL (cmd)" produces). Never throws — an unterminated quote
/// simply runs to the end of input rather than being rejected, since this is
/// used to parse commands pasted by a human, not to validate shell syntax.
/// Shared by curl-convert and docker-convert (both need argv-style splitting
/// of a single command line into flags/values).
/// </summary>
public static class ShellTokenizer
{
    private enum Mode { Normal, Single, Double }
